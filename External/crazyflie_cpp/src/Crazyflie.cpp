...
Crazyflie::Crazyflie(int radioIndex, int channel, Crazyradio::Datarate baudRate, const String &address) :
	m_radio(nullptr)
	, m_transport(nullptr)
	, m_devId(radioIndex)
	, m_channel(channel)
	, m_address(address.getHexValue64())
	, m_datarate(baudRate)
	, m_logTocEntries()
	, m_logBlockCb()
	, m_paramTocEntries()
	...
	, m_consoleCallback(nullptr)
{
	...
	else { //if radioIndex is set to -1, use the channel as usb index
		m_devId = m_channel;
		if (m_devId >= MAX_USB) {
	...
	const std::vector<uint8_t>& data)
{
	...
	for (uint16_t page = flashStart; page < numPages + flashStart; ++page) {
		for (uint16_t address = 0; address < pageSize; address += 25) {
			...
			size_t requestedSize = std::fmin(data.size() - offset, std::fmin(25, pageSize - address));
