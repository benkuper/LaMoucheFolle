...
void OSCController::sendDroneFeedback(Drone * d, Controllable * c)
{
	...
	if (p != nullptr)
	{
		OSCMessage m(p->getControlAddress());
...
void OSCController::processMessage(const OSCMessage & msg)
{
	...
	else if (tokens[1].contains("drones"))
	{
		if (tokens.size() < 2)
		{
		...
		if(tokens[2] == "setup") sendDroneSetup(droneId);
		else
		{
			Drone * d = DroneManager::getInstance()->getDroneWithID(droneId.getIntValue()); 
			if (d == nullptr)
			{
				LOGWARNING("Could not find drone " << droneId << "( address : " << msg.getAddressPattern().toString() << ")");
				return;
			}

			String s = "/"+tokens[3];
			for (int i = 4; i < tokens.size(); i++) s += "/"+tokens[i];
