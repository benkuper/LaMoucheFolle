...
public:
	...
	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CFParam)
...
public:
	...
	HashMap<int, CFParam *> paramIdsMap;

	var getParamValue(const String &name);
	var getParamValue(int id);
	int getParamIdForName(const String &name);
	String getParamNameForId(int  id) const;
	CFParam * getParam(const String &name);
	CFParam * getParam(int id);

	Array<int> getMissingIds();
	...
	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CFParamToc)
};