...
public:
	...
	void addVariableDef(const String &name, int id, CFLogVariable::Type type);

	void save();

	OwnedArray<CFLogVariable> variables;
	HashMap<String, CFLogVariable *> variableNamesMap;
	HashMap<int, CFLogVariable *>variableIdsMap;

	var getLogVariableValue(const String &name);
	var getLogVariableValue(int id);
	int getLogVariableIdForName(const String &name);
	String getLogVariableNameForId(int  id) const;
	CFLogVariable * getLogVariable(const String &name);
	CFLogVariable * getLogVariable(int id);
