...
const uint8_t CFPacket::safeLinkPacket[3] = {0xFF, 0x05, 0x01};

CFPacket::CFPacket(Drone * drone, const ITransport::Ack & ack)
{
	hasSafeLink = memcmp(ack.data, safeLinkPacket, 3) == 0;
	...
	if (crtpConsoleResponse::match(ack)) {
	...
	} else if (crtpLogGetInfoV2Response::match(ack)) {
		type = LOG_TOC_INFO;
		crtpLogGetInfoV2Response * r = (crtpLogGetInfoV2Response *)ack.data;
	...
	} else if (crtpLogGetItemV2Response::match(ack)) {
		type = LOG_TOC_ITEM;
		crtpLogGetItemV2Response * r = (crtpLogGetItemV2Response *)ack.data;
		data = new DynamicObject();
	...
	} else if (crtpParamTocGetInfoV2Response::match(ack)) {
		type = PARAM_TOC_INFO;
		crtpParamTocGetInfoV2Response * r = (crtpParamTocGetInfoV2Response *)ack.data;
		data = new DynamicObject();
		data.getDynamicObject()->setProperty("crc", (int)r->crc);
		data.getDynamicObject()->setProperty("size", r->numParam);

	} else if (crtpParamTocGetItemV2Response::match(ack)) {
		type = PARAM_TOC_ITEM;
		crtpParamTocGetItemV2Response * r = (crtpParamTocGetItemV2Response *)ack.data;
		data = new DynamicObject();
		String groupName = String(&r->text[0]);
	...
	}  else if (crtpParamValueV2Response::match(ack)) {
		type = PARAM_VALUE;

		crtpParamValueV2Response * r = (crtpParamValueV2Response *)ack.data;
		data = new DynamicObject();
		...
		if (p == nullptr)
		{
			DBG("TOC has not param with id : " << (int)r->request.id);
			return;
		}
