...
{
	CFParam* p = getParam(name);
	if (p != nullptr)
	{
		DBG("WEIRD : Param " << name << " already exists in this TOC, existing id : "  << p->definition.id << ", new id " << id);
		//return;
	}

	p = new CFParam(CFParam::Definition(name, type, id));
	params.add(p);
...
var CFParamToc::getParamValue(int id)
{
...
String CFParamToc::getParamNameForId(int id) const
{
...
CFParam * CFParamToc::getParam(int id)
{
...
Array<int> CFParamToc::getMissingIds()
{
	Array<int> result;
